...
# data cleaning for plotting percentage of each race and voters over total population
ns3_1 <- ns3 %>%
...
ns3_2 <- gather(ns3_1, Category, Population, -c(GEONAME, LNTITLE, GEOID, LNNUMBER, 
                                                Percent_ADU, Percent_CIT, 
                                                Percent_CVAP, Percent_of_Total, 
                                                Percent_of_Voter_Total))

ns3_3 <- gather(ns3_2, Code, Percent, -c(GEONAME, LNTITLE, GEOID, LNNUMBER, 
                                         Category, Population))

ns3_4 <- ns3_3 %>%
        select(-GEOID) %>%
        mutate(Category = case_when(
                Category == "TOT_EST" ~ "Total_Population",
                Category == "ADU_EST" ~ "Adult_Population",
                Category == "CIT_EST" ~ "Citizen_Population",
                Category == "CVAP_EST" ~ "Adilt_Citizen_Population"
        )) %>%
        mutate(Code = case_when(
                Code == "Percent_ADU" ~ "Adult_Percentage_in_Race",
                Code == "Percent_CIT" ~ "Citizen_Percentage_in_Race",
                Code == "Percent_CVAP" ~ "Adult_Citizen_Percentage_in_Race",
                Code == "Percent_of_Total" ~ "Race_Percentage_of_Total_Population",
                Code == "Percent_of_Voter_Total" ~ "Adult_Citizen_Percentage_of_Total_Population"
        ))

# plotting for percentage of each race and voters over total population 
ns3_5 <- ns3_4 %>%
        select(-c(Category, Population)) %>%
        filter(Code %in% c("Race_Percentage_of_Total_Population", "Adult_Citizen_Percentage_of_Total_Population"))
